...
# ── CSS inject สำหรับ container border-left ──────────────────
_CARD_CSS = """
<style>
...
def _card_title(text: str):
    """Section title ภายใน container"""
    st.markdown(
        f'<div class="rp-card-title">{text}</div>',
...
def render_tab1():
    ...
    # ════════════════════════════════════════════════════════
    with st.container(border=True):
        _card_title('📊 นำเข้า ESAL จาก ESAL Calculator')
        ...
        if ed:
            ...
            st.markdown(
                '<div class="rp-status-info">⚪ ยังไม่ได้นำเข้าข้อมูล ESAL</div>',
                unsafe_allow_html=True)

    # ════════════════════════════════════════════════════════
    # Card 3 — Serviceability & Reliability
    # ════════════════════════════════════════════════════════
    with st.container(border=True):
        _card_title('📉 Serviceability & Reliability')
        c1, c2, c3 = st.columns(3)

        with c1:
            pt = st.slider(
                'Terminal Serviceability (Pt)', 1.5, 3.0,
            ...
            if ed:
                ...
                if abs(pt - pt_json) > 0.01:
                    st.warning(
                        f'⚠️ pt ที่ใช้ ({pt:.1f}) ต่างจาก ESAL JSON ({pt_json:.1f})\n\n'
                        f'W18 จะถูกคำนวณใหม่ด้วย pt = {pt:.1f}')
                    if st.button(
                            f'↩️ ใช้ค่าจาก ESAL JSON (pt = {pt_json:.1f})',
                            key='reset_pt'):
        ...
        with c3:
            so = st.number_input(
                'Overall Std. Deviation (So)', 0.30, 0.45,
                st.session_state.get('so', 0.35), 0.01,
                format='%.2f', key='so')

    # ════════════════════════════════════════════════════════
    # Card 4 — W18 Summary (แสดงเฉพาะเมื่อมี ESAL data)
    # ════════════════════════════════════════════════════════
    if ed:
        with st.container(border=True):
            _card_title('🔢 W18 ตามความหนาคอนกรีต D')
