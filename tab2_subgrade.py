...
def render_tab2():
    ...
    with st.container(border=True):
        ...
        with c2:
            st.markdown(
                ...
                f'<div style="font-size:11px;color:#78909C">{formula}</div>'
                f'<div style="font-family:IBM Plex Mono,monospace;font-size:20px;'
                f'font-weight:700;color:{_JPCP_BD}">{MR_psi:,.0f} psi</div>'
                f'<div style="font-size:11px;color:#78909C">({MR_psi/145.038:.1f} MPa)</div>'
                f'</div>', unsafe_allow_html=True)
        with c3:
            st.markdown(
                f'<div style="background:#F0F7FF;border:1px solid {_JPCP_BDLT};'
                f'border-radius:8px;padding:10px 14px;margin-top:4px;'
                f'font-size:13px;color:{_JPCP_BD}">'
                f'ℹ️ MR = <strong>{MR_psi:,.0f} psi</strong> ใช้กับทั้ง JPCP และ CRCP'
                f'</div>', unsafe_allow_html=True)
        st.session_state['MR_psi'] = MR_psi

    # ════════════════════════════════════════════════════════
    # Row A — Layers (JPCP | CRCP)
    # ════════════════════════════════════════════════════════
    ...
    with col_c:
        ...
        with st.container(border=True):
            ...
            if copy_jpcp:
                ...
                st.markdown(
                    f'<div style="font-size:12px;color:{_CRCP_BD};background:{_CRCP_BG};'
    ...
    if kj or kc:
        ...
        with st.container(border=True):
            ...
            with s_left:
                st.markdown(
                    f'<div style="font-size:13px;font-weight:700;color:{_CRCP_BD};'
                    f'padding:6px 0">✅ สรุป k_eff → ส่งต่อ Tab 3 Design</div>',
                    unsafe_allow_html=True)
            with s_right:
                sc1, sc2 = st.columns(2)
                with sc1:
                    if kj:
                        _mbox('k_eff — JPCP/JRCP', f'{kj:.0f}', 'pci', _JPCP_BD, _JPCP_BG)
                with sc2:
                    if kc:
                        _mbox('k_eff — CRCP', f'{kc:.0f}', 'pci', _CRCP_BD, _CRCP_BG)
